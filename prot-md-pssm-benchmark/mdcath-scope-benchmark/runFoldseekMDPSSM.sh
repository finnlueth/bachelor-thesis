...
# python ../scope-benchmark-minimal/generate_foldseek_db_new.py ${SEQUENCE_FASTA_AA} ${SEQUENCE_FASTA_3Di} ${DB_SEQUENCE}/${DB_NAME_FOLDSEEK} ${OUT_DIR}
...
# Profile vs Sequence
${FOLDSEEK} search ${DB_SEQUENCE}/${DB_NAME_FOLDSEEK} ${DB_PROFILE}/${DB_NAME_FOLDSEEK}_profile ${ALIGN_DIR_ALN} ${ALIGN_DIR_TMP} --threads 8 -s 9.5 --max-seqs 2000 -e 10

# Convert the alignment
${FOLDSEEK} convertalis ${DB_SEQUENCE}/${DB_NAME_FOLDSEEK} ${DB_PROFILE}/${DB_NAME_FOLDSEEK}_profile ${ALIGN_DIR_ALN} ${ALIGN_DIR_RAW}/foldseekaln
