...
# write TSV files
with open("{}/aa.tsv".format(tmp_dir), "w") as f:
    f.write(tsv_aa)
with open("{}/3di.tsv".format(tmp_dir), "w") as f:
    f.write(tsv_3di)
with open("{}/header.tsv".format(tmp_dir), "w") as f:
    f.write(tsv_header)

# create Foldseek database
os.system("mmseqs tsv2db {}/aa.tsv {} --output-dbtype 0".format(tmp_dir, db_name))
os.system("mmseqs tsv2db {}/3di.tsv {}_ss --output-dbtype 0".format(tmp_dir, db_name))
os.system("mmseqs tsv2db {}/header.tsv {}_h --output-dbtype 12".format(tmp_dir, db_name))
